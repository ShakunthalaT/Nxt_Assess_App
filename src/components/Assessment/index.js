...
class Assessment extends Component {
        ...
        {optionsType === 'SINGLE_SELECT' && (
          <>
            ...
            <div className="select-text-container">
              <div className="selected-by-default">
                <IoIosAlert size={25} className="alert" />
  ...
  renderOptions = options => {
    ...
    return options.map(option => (
      <li key={option.optionId} className="normal-list-options">
        <button
          type="button"
  ...
  renderImageOptions = options => {
    ...
    return options.map(option => (
      <li key={option.optionId}>
        <img
          className={
            selectedOption === option.optionId ? 'selected-img' : 'normal-img'
          }
          onClick={() => this.onClickAnswer(option.optionId)}
          src={option.imageUrl}
          alt={option.text}
